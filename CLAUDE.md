...
is `venv/` at the root, on **Python 3.14**. (A duplicate `backend/venv/` used
to exist and was removed — if it reappears, something recreated it in the wrong
directory.)
...
| Dev servers | `/dev` |
| Sync venv to the lock | `venv/bin/uv pip sync requirements-dev.lock` |
...
`backend/requirements.txt` and `requirements-dev.txt` hold **ranges** and are
the human-edited inputs. Nothing installs from them any more. The installed
sets are `backend/requirements.lock` (runtime) and `requirements-dev.lock`
(superset, adds pytest/ruff/uv) — fully pinned with hashes, generated by `uv`,
and used by all three consumers:

- local venv → `venv/bin/uv pip sync requirements-dev.lock`
- CI → `pip install --require-hashes -r requirements-dev.lock`
- container → `pip install --require-hashes -r requirements.lock`

**Why this matters here and not just as hygiene.** Before locking, `venv/` had
...
To change a dependency: edit the `.txt`, regenerate **both** locks, sync, run
the gate.

```
venv/bin/uv pip compile backend/requirements.txt -o backend/requirements.lock --generate-hashes --python-version 3.14
venv/bin/uv pip compile requirements-dev.txt    -o requirements-dev.lock    --generate-hashes --python-version 3.14
venv/bin/uv pip sync requirements-dev.lock
```

`uv` lives **in the venv**, not on the machine, and is listed in
`requirements-dev.txt` so `uv pip sync` does not uninstall itself. There is no
lock-drift check in CI on purpose: `uv pip compile` re-resolves to latest, so a
diff-based check would go red whenever any upstream publishes a release, with
no change in this repo. The guarantee is the hash-checked install, not a diff.
